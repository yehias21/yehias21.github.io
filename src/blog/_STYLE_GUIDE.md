...
3. Add an entry to `BLOG_POSTS` (newest first):
   ...
   (`/blog/foo.png`), never `./blog/...` — relative paths break on the
   `/blog/:id` route.
...
- **Section hierarchy:** `##` for numbered top-level sections, `###` for
  subsections. Keep headings terse — they populate the on-page TOC.
...
<!-- Opening: 1–2 paragraphs of motivation. State the question the post answers
     and why the reader should care. No heading here. -->
...
citation — **Some Method** (Author 2024) — and refer back to it as
